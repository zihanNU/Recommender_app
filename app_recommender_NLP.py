...
if __name__ == '__main__':
    ...
    m=TicToc()
    m.tic()
    results =pd.DataFrame(columns=['loadID', 'Reason', 'Score','origin','destination','truckId','Carrier_Origin','Carrier_Destination'])
    if (trucks.loc[0]['carrierID'])>0:
        carrier_load = Get_Carrier_histLoad(int(trucks.loc[0]['carrierID']))
    else:
        carrier_load = {'flag': 0, 'histload': 0}
    for i in range(19,len(trucks)):
        truck=trucks.iloc[i]
        ...
        t.toc('histload')
        if i>1 and trucks.iloc[i]["truckId"]==trucks.iloc[i-1]["truckId"]:
        ...
        if i==len(trucks)-1:
            results.iloc[0:10].to_csv("results_NLP"+str(truck['carrierID'])+".csv",mode = 'a+', header=False,index=False)
        elif trucks.iloc[i]["truckId"]!=trucks.iloc[i+1]["truckId"]:
            if not os.path.isfile("results_NLP"+str(truck['carrierID'])+".csv"):
                results.iloc[0:10].to_csv("results_NLP" + str(truck['carrierID']) + ".csv",index=False)
            else:
                results.iloc[0:10].to_csv("results_NLP"+str(truck['carrierID'])+".csv",mode = 'a+', header=False,index=False)
