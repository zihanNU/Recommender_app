...
def Get_truck_NLP():
    ...
    SELECT   [truckId]
      ,[numberOfTruck]
      ,[originCity]
      ,UPPER([OriginState]) 'originState'
      ,destinationCity  'destinationCity'
      ,UPPER([destinationState]) 'destinationState'
	  ,[originCity]+','+UPPER([OriginState]) 'Carrier_Origin'
	  ,destinationCity+','+UPPER([destinationState]) 'Carrier_Destination'
      ,convert(date,[emptyDate])   'EmptyDate'
      ,[carrierId]   'carrierID'
      ,[lat_Origin]  'originLat'
      ,[lng_Origin]  'originLon'
      ,[lat_dest]   'destLat'
      ,[lng_dest]   'destLon'
	  ,COALESCE (case when cargolimit= 0 then 500000 else cargolimit end, 500000)  'cargolimit'
	  ,'' 'EquipmentType'
	  ,53 'EquipmentLength'
	  ,0 'originDeadHead_radius'
	  ,0 'destinationDeadHead_radius'
    FROM [ResearchScience].[dbo].[NLP_TRUCK] T
    left join bazooka.dbo.Carrier Car on Car.id= T.[carrierId]

            """
    trucks_df = pd.read_sql(sql=sql, con=cn)
    return trucks_df


def Get_truck_Evaluate(date):
    cn = pyodbc.connect('DRIVER={SQL Server};SERVER=ANALYTICSDev;DATABASE=ResearchScience;trusted_connection=true')
    sql = """
   SELECT top 1000  [truckid]
      ,[carrierID]
      ,convert(date, emptyData)   'EmptyDate'
      ,[originState]
      ,[destinationState]
      ,[originLat]
      ,[originLon]
      ,[destLat]
      ,[destLon]
      ,[originDeadHead_radius]
      ,[destinationDeadHead_radius]
      ,[originCluster]
      ,[destinationCluster]
      ,COALESCE (case when cargolimit= 0 then 500000 else cargolimit end, 500000)  'cargolimit'
  FROM [ResearchScience].[dbo].[Recommendation_Trucks_Records] T
  left join bazooka.dbo.Carrier Car on Car.id= T.[carrierId]
  where convert(date, emptyData) =?
  and carrierID in (select carrierID from [ResearchScience].[dbo].[Recommendation_ActiveCarriers] )
  order by carrierID
            """
    trucks_df = pd.read_sql(sql=sql, con=cn,params=[date])
    return trucks_df
...
# Give CarrierID
def Get_Carrier_histLoad(CarrierID,date):
    ...
    query = """
        ...
        declare @date1 as date = dateadd(day,-97,@date )
        declare @date2 as date =  dateadd(day,-7,@date )
        select * from [ResearchScience].[dbo].[Recommendation_HistLoads]
        where CarrierID= @CarrierID  
        and loaddate between @date1 and @date2
	"""

    histload = pd.read_sql(query, cn, params=[CarrierID,date])
...
def Get_newload_NLP(date1, carrierlat, carrierlon, cargolimit):
    ...
    declare @date2 as date = dateadd(day,1,@date1)
	...
	where L.StateType = 1 --and L.progresstype=1 
	and L.totalrate>150
    ...
    L.OriginStateCode  'originState',
	L.DestinationCityName + '-'+L.DestinationStateCode 'destination',
	L.DestinationStateCode 'destinationState',
   ...
   where 
    ...
    and CityO.Latitude between @carrierlat-10 and @carrierlat+10 
    and CityO.Longitude between  @carrierlon-5 and  @carrierlon+5
    and value <= @cargolimit
    """
    newload = pd.read_sql(query, cn, params=[date1, carrierlat, carrierlon, cargolimit])
...
def makeMatrix(x, y, z):  # x is the hist load list, y is the unique ode list; x and y are pd.df structure
    ...
    for j in y.itertuples():
        ...
        if (len(selectedloads) > 0):
            ...
            kpiMatrix.append(
                carrier_ode_loads_kpi_std(z, j, loads, np.mean(np.asarray(std1)), np.std(np.asarray(std1))))
...
def get_odelist_hist(loadlist):
    ...
    for x in loadlist.itertuples():
        ...
        #                 })
        odelist.append({'origin': x.originCluster, 'destination': x.destinationCluster, 'corridor': x.corridor,
                        'equipment': x.equipment, 'origin_count': x.origin_count, 'origin_max': x.origin_count,
                        'dest_count': x.dest_count, 'dest_max': x.dest_max
                        })
...
def find_ode(kpilist, load, odlist):
    ...
    if len(
            matchlist) > 0:  # merge match list, if we find mutiple matches for either origin cluster or destination cluster
        matchlist_merge = []
        perc_merge = []
...
def dynamic_input(newloads_df, carrier):
    ...
    if carrier.originLat is not None and carrier.originLon is not None:
        ...
        #if carrier.originCity is '':
        #    newloads_df['originDH'][newloads_df['originState'] == carrier.originState] = 0
    ...
    if carrier.destLat is not None and carrier.destLon is not None:
        ...
        #if carrier.destinationCity is '':
        #    newloads_df['destDH'][newloads_df['destinationState'] == carrier.destinationState] = 0
...
def reasoning(results_df):
    ...
    reason_label = ['close to origin', 'short total deadhead', 'good historical performance on similar loads',
                    'estimated margin', 'close to pickup time', 'desired OD']
...
def recommender(carrier_load, trucks_df):
    ...
    newloads_df = Get_newload_NLP(carrier.EmptyDate, carrier.originLat, carrier.originLon, carrier.cargolimit)
    ...
    if len(newloads_df) > 0:
        ...
        if len(newloads_select) > 0:
            ...
            results_df['Score'] = (results_df['ODH_Score'] * 0.25 + results_df['totalDH_Score'] * 0.20 + \
                                   ...
                                   results_df['puGap_Score'] * 0.05 + results_df['desired_OD'] * 0.1).astype('int')
            results_df['Reason'] = reasoning(results_df)
            results_sort_df = results_df[results_df.Score > 0].sort_values(by=['Score'], ascending=False)
            t.tic()

            result_json = results_sort_df[['loadID', 'Reason', 'Score', 'origin', 'destination', 'pu_appt']]
            #result_json['truckId'] = carrier['truckId']
            result_json['EmptyDate'] = carrier['EmptyDate']
            #result_json['Carrier_Origin'] = carrier['Carrier_Origin']
            #result_json['Carrier_Destination'] = carrier['Carrier_Destination']
...
def merge_results(results):
    ...
    #origins = ';'.join(m for m in list(set(results_sort['Carrier_Origin'])))
    #results_sort['Carrier_Origin'] = origins
    #dests = ';'.join(m for m in list(set(results_sort['Carrier_Destination'])))
    #results_sort['Carrier_Destination'] = dests
    results_df = results_sort.drop_duplicates(subset='loadID', keep="first")

    return (results_df)



def Get_realload(CarrierID,date):
    cn = pyodbc.connect('DRIVER={SQL Server};SERVER=ANALYTICSDev;DATABASE=ResearchScience;trusted_connection=true')
    query = """
        set nocount on
        declare @CarrierID as int =?   
        declare @date as date = ?
        declare @date1 as date = @date
        declare @date2 as date =  dateadd(day,1,@date )
        select * from [ResearchScience].[dbo].[Recommendation_HistLoads]
        where CarrierID= @CarrierID  
        and loaddate between @date1 and @date2
	"""
...
if __name__ == '__main__':
    t = TicToc()
    t.tic()
    #trucks = Get_truck_NLP()
    date='2018-12-05'
    trucks = Get_truck_Evaluate(date)
    t.toc('truck')
    t.tic()
    m = TicToc()
    m.tic()
    results = pd.DataFrame(columns=['loadID', 'Reason', 'Score', 'origin', 'destination', 'truckId', 'Carrier_Origin',
                                    'Carrier_Destination'])
    if (trucks.loc[0]['carrierID']) > 0:
        carrier_load = Get_Carrier_histLoad(int(trucks.loc[0]['carrierID']),date)
        realloads_df = Get_realload(int(trucks.loc[0]['carrierID']),date)
    else:
    ...
    for i in range(0, len(trucks)):
        ...
        t.toc('histload')
        if i > 1 and trucks.iloc[i]["carrierID"] == trucks.iloc[i - 1]["carrierID"]:
            reclist = recommender(carrier_load, carrier_df)
            results = results.append(reclist)
            results = merge_results(results)
        else:
            results = recommender(carrier_load, carrier_df)
        if i == len(trucks) - 1:
            results[results['Score']>30].to_csv("D:/Git/recommender/evaluate/results_EVA" + str(truck['carrierID']) + ".csv", mode='a+', header=False,
                                      index=False)
            realloads_df.to_csv(
                "D:/Git/recommender/evaluate/results_EVA" + str(truck['carrierID']) + ".csv", mode='a+', header=False,
                index=False)
        elif trucks.iloc[i]["carrierID"] != trucks.iloc[i + 1]["carrierID"]:
            if not os.path.isfile("D:/Git/recommender/evaluate/results_EVA" + str(truck['carrierID']) + ".csv"):
                results[results['Score']>30].to_csv("D:/Git/recommender/evaluate/results_EVA" + str(truck['carrierID']) + ".csv", index=False)
            else:
                results[results['Score']>30].to_csv("D:/Git/recommender/evaluate/results_EVA" + str(truck['carrierID']) + ".csv", mode='a+', header=False,
                                          index=False)
            realloads_df.to_csv(
