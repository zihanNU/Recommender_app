...
def Get_newload(date1,date2):
	...
	left join Analytics.bazooka.dbo.DandBIndustry D  on C.DandBIndustryId=D.DandBIndustryId
   where 
...
def check(carrier_load,newloads,carrier):
    ...
    newloads.update(pd.DataFrame(carrier_load_score))
    return (newloads)
...
def new_hist_score(newload,kpiMatrix, kpi_odlist,num_load):
    new_ode={'origin':newload.originCluster,'destination':newload.destinationCluster,'corridor':newload.corridor,'equipment':newload.equipment}
    matchlist,   weight, corridor_vol = find_ode(kpiMatrix,new_ode,kpi_odlist )
    if len(matchlist) > 0:
        score = similarity(matchlist, newload, weight)
        score['desired_OD'] = 100 if corridor_vol > min(num_load * 0.1, 10) else 0
    else:
        score = {'carrierID': carrierID,
...
def indiv_recommender(carrier,newloads,loadList):
    ...
    t.toc('kpimatrix')
    t.tic()
    carrier_load_score={'hist_perf': newloads.apply(lambda row:new_hist_score(row,kpiMatrix, kpi_odlist,len(loadList)),axis=1)}
    #newloads.update(pd.DataFrame(carrier_load_score))
    t.toc('scoring')
    return (carrier_load_score)
...
def recommender( carrier_load,trucks_df):
    ...
    if len(newloads_df) > 0:
        ...
        if len(newloads_select) > 0:
            t.tic()
            newloads_select['hist_perf']=0
            newloads_select = check(carrier_load, newloads_select,carrier)
            t.toc('hist_scoring')
            t.tic()
            #results_df = pd.DataFrame(carrier_load_score).merge(newloads_select, left_on="loadID", right_on="loadID",how='inner')
            corridor_info = read_corridor()
            results_df = newloads_select.merge(corridor_info, left_on='corridor', right_on='corridor', how='left')
